...
"use strict";
    ...
    /* Antox code */
    var labarreboutons = $('<div class="col-md-12 lesboutons"><div class="col-md-6" id="enregistrer"></div><div class="col-md-6" id="afficher"></div></div>');
  
    // Editlayout
  
    // Edit    
    if(document.getElementById('editview_title_container')){
        labarreboutons.insertBefore($('#editview_title_container'));
        $('#afficher').append($('#editview_submit_container'));
        $('#editview_submit_container #editview_submit').text('Etape Suivante').val('Etape Suivante').addClass('pull-right');
        $('#enregistrer').append($('#display_page'));
    }
    
    // Layout
    if(document.getElementById('viewlayout_basic_container')){
        labarreboutons.insertBefore($('#viewlayout_basic_container'));
        $('#afficher').append($('#exportsubmitcontainer'));
        $('#viewlayout_submit').addClass('pull-right').text('Etape Suivante').val('Etape Suivante');
        $("#enregistrer").append($('#display_page'));
    }
    
    // Blocks
    if(document.getElementById('blocksinstruction')){
        labarreboutons.insertBefore($('#blocksinstruction'));
        $('#enregistrer').append($('#view-wizard-controls'));
        $('#afficher').append($('.leboutonquivabien'));
        $('.leboutonquivabien').addClass("btn").addClass("btn-default");
        $('.leboutonquivabien').addClass('pull-right').text('Etape Suivante');
    }
    
    // Share
    if(document.getElementById('lesuperaccessurl')){
        labarreboutons.insertBefore($('#lesuperaccessurl'));
        $('#enregistrer').append($('#view-wizard-controls'));
    }
    // Access
     if($('#lesuperaccessurl').length !== 0){
         $('.section-heading:first').replaceWith($('.letitreadeplacer')); // Titre de la page
     }
    ...
    * Institutions
   *************************************/
    var switchtheme = "#sb-switchtheme"; 
    
    $("#sb-switchtheme #dialoginst").hide(); 
    
    $('.list-group-item.bouton-instit-theme.laboiteaafficher').mouseover(function(){
        $("#sb-switchtheme #dialoginst").show(); 
    });
    $('.list-group-item.bouton-instit-theme.laboiteaafficher').mouseleave(function(){
        $("#sb-switchtheme #dialoginst").hide(); 
    });
